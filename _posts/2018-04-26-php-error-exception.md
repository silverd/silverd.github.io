...
(2) `\ErrorException` 是继承于 `\Exception` 的异常子类，扩展了更多的参数，例如文件名和行号。一般专门用在 `set_error_handler()` 或者 `register_shutdown_function()` 将错误转化为异常时，因为普通异常只有 code/message 两个属性，不便排查。
